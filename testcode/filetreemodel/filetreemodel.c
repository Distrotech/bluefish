...
print_record(UriRecord *record, gint indent)
{
	gint i;
	pindent(indent);
	g_print("'%s' (%p), isdir=%d, mime-type=%s", record->name,record,record->isdir,g_file_info_get_attribute_string(record->finfo, G_FILE_ATTRIBUTE_STANDARD_FAST_CONTENT_TYPE));
	if (record->num_rows) {
...
filetreemodel_iter_children(GtkTreeModel * tree_model, GtkTreeIter * iter, GtkTreeIter * parent)
{
	...
	if (parent == NULL) {
		...
		g_print("filetreemodel_iter_children, return first child for '%s'\n",record->name);
		iter->stamp = filetreemodel->stamp;
...
static gint filetreemodel_iter_n_children(GtkTreeModel * tree_model, GtkTreeIter * iter)
{
	...
	g_print("filetreemodel_iter_n_children return %d for '%s'\n",record->num_rows,record->name);
...
filetreemodel_iter_nth_child(GtkTreeModel * tree_model, GtkTreeIter * iter, GtkTreeIter * parent, gint n)
{
	...
	if (parent) {
		...
		g_print("filetreemodel_iter_nth_child(%d) for '%s'\n",n,precord->name);
		if (n >= precord->num_rows)
			return FALSE;
		record = precord->rows[n];
	} else {
		/* special case: if parent == NULL, set iter to n-th top-level row */
		g_print("filetreemodel_iter_nth_child(%d) for root\n",n);
...
static void
fill_uri(UriRecord *newrecord, GFile *uri, GFileInfo *finfo)
{
	GIcon *icon;
	...
	if (newrecord->isdir && strcmp(g_file_info_get_attribute_string(newrecord->finfo, G_FILE_ATTRIBUTE_STANDARD_FAST_CONTENT_TYPE), "inode/directory")!=0) {
		g_print("%s: isdir=%d but mime type =%s ???????????\n",newrecord->name,newrecord->isdir,g_file_info_get_attribute_string(newrecord->finfo, G_FILE_ATTRIBUTE_STANDARD_FAST_CONTENT_TYPE));
		g_assert_not_reached();
	}
...
static void
add_dummy_subdir(FileTreemodel * filetreemodel, UriRecord *precord)
{
	GFile *dummy_uri;
	...
	g_print("add_dummy_subdir, precord %p '%s', dummy_uri=%p, dummy_finfo=%p\n",precord,precord->name,dummy_uri,dummy_finfo);
	add_single_uri(filetreemodel, precord, dummy_uri, dummy_finfo);
...
UriRecord *filetreemodel_build_dir(FileTreemodel * filetreemodel, GFile *uri)
{
	...
	while (!done) {
		...
		if (g_file_equal(tmp2, uri)) {
			add_dummy_subdir(filetreemodel, record);
		}
		g_object_unref(finfo);
	...
	while(tmplist) {
		...
		if (tmp) {
			...
			if (g_file_info_get_file_type(newrecord->finfo)==G_FILE_TYPE_DIRECTORY) {
				add_dummy_subdir(filetreemodel, newrecord);
			}
