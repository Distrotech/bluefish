/* compile with
gcc -g -Wall -O2 -o filetreemodel filetreemodel.c main.c `pkg-config --cflags --libs gtk+-3.0`

G_SLICE=always-malloc G_DEBUG=gc-friendly valgrind --tool=memcheck --num-callers=32 ./filetreemodel

indent -ts4 -kr -l110 *.c
...
/*	const gchar *firstnames[] = { "Joe", "Jane", "William", "Hannibal", "Timothy", "Gargamel", NULL };
	...
	const gchar *paths[] =
		{ "/home/olivier/svnbluefish", "/home2/olivier/", "file://home/olivier/tmp",
"sftp://olivier.sessink.nl/var/www", NULL };
	while (paths[i]) {
...
static void view_row_expanded(GtkTreeView * tree, GtkTreeIter * iter, GtkTreePath * path, FileTreemodel * ftm)
{
	filetreemodel_refresh_iter_async(ftm, iter);
}

static void view_selection_changed(GtkTreeSelection * treeselection, FileTreemodel * ftm)
{
	GtkTreeModel *model;
	GtkTreeIter iter;
...
GtkWidget *create_view_and_model(void)
{
	...
	gtk_tree_view_column_set_attributes(col, renderer, "icon-name", filetreemodel_COL_ICON_NAME,
										/*"pixbuf_expander_closed", PIXMAP_COLUMN,
										   "pixbuf_expander_open", PIXMAP_COLUMN, */ NULL);

	renderer = gtk_cell_renderer_text_new();
	gtk_tree_view_column_pack_start(col, renderer, TRUE);
	gtk_tree_view_column_set_attributes(col, renderer, "text", filetreemodel_COL_NAME, "weight",
										filetreemodel_COL_WEIGHT, NULL);
