		...
		if (prefix && *prefix != '\0') {
			...
			if (items) {
				...
				if (gtk_dialog_run(GTK_DIALOG(acw->win)) ==GTK_RESPONSE_OK ) {
					...
					GtkTreeIter it;
					GtkTreeModel *model;
					DBG_MSG("autocomp_run, do something\n");
