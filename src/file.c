...
static gboolean open_file_channel_lcb(GIOChannel *source,GIOCondition condition,gpointer data) {
	...
	if (condition & G_IO_IN) { /* Data is available. */
		...
		g_io_channel_read(source, chunk,CHUNK_SIZE,&bytes_read);
		...
		DEBUG_MSG("open_file_channel_lcb, block read, used_size=%lld\n",of->used_size);
	}
...
static void open_file_as_channel_lcb(GnomeVFSAsyncHandle *handle,GIOChannel *channel, GnomeVFSResult result, gpointer data) {
	...
	if (result == GNOME_VFS_OK) {
		...
		g_io_add_watch_full(channel, 0,G_IO_IN|G_IO_NVAL|G_IO_HUP,open_file_channel_lcb,of,NULL);
	} else {
		DEBUG_MSG("open_file_as_channel_lcb, error, cleanup %p\n",of);
...
static void file_open_uri_async(GnomeVFSURI *uri, OpenfileAsyncCallback callback_func, gpointer callback_data) {
	...
	gnome_vfs_async_open_uri_as_channel(&handle, uri,GNOME_VFS_OPEN_READ, CHUNK_SIZE
...
static void file2doc_lcb(gint status,gint error_info,gchar *buffer,gpointer data) {
	DEBUG_MSG("file2doc_lcb, status=%d, data=%p\n",status,data);
	switch (status) {
		case OPENFILE_FINISHED:
			DEBUG_MSG("file_to_doc_lcb, status=%d, now we should convert the data into a GtkTextBuffer and such\n",status);
			DEBUG_MSG(buffer);
		...
		case OPENFILE_ERROR_NOREAD:
			DEBUG_MSG("file_to_doc_lcb, status=%d, cleanup!!!!!\n",status);
			file2doc_cleanup(data);
		break;
	}
}
...
void file_doc_from_uri(Tbfwin *bfwin, GnomeVFSURI *uri) {
	Tfile2doc *f2d;
	DEBUG_MSG("file_doc_from_uri, open %s\n", gnome_vfs_uri_get_path(uri));
