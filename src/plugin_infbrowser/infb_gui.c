...
*/
static gboolean
infb_motion_notify_event(GtkWidget * text_view, GdkEventMotion * event, gpointer user_data)
{
	gint x, y;
	...
	gdk_window_get_pointer(gtk_text_view_get_window(GTK_TEXT_VIEW(text_view), GTK_TEXT_WINDOW_WIDGET), NULL,
						   NULL, NULL);
...
*/
gboolean
infb_button_release_event(GtkWidget * widget, GdkEventButton * event, gpointer user_data)
{
	gint x, y;
	...
	if (event->button == 1) {
		...
		if (gtk_text_iter_get_offset(&start) != gtk_text_iter_get_offset(&end))
			return FALSE;
		gtk_text_view_window_to_buffer_coords(GTK_TEXT_VIEW(widget), GTK_TEXT_WINDOW_WIDGET, event->x,
											  event->y, &x, &y);
				...
				if (aux) {
					...
					if (aux) {
						...
						if (g_str_has_prefix((gchar *) aux, "http://")) {
							gchar *pstr =
								g_strdup_printf("%s/bfish_%ld", g_get_tmp_dir(), (long int) time(NULL));
							gchar *info;
							if (xmlNanoHTTPFetch((const char *) aux, pstr, &info) == -1) {
								g_free(pstr);
								infb_insert_error(GTK_TEXT_VIEW(widget),
												  BAD_CAST _("Cannot load file from network"));
							...
							if (doc)
								doc->URL = xmlStrdup(BAD_CAST aux);
							g_free(pstr);
						} else {
							if (!g_file_test((gchar *) aux, G_FILE_TEST_EXISTS)
								|| !g_file_test((gchar *) aux, G_FILE_TEST_IS_REGULAR)) {
							...
							if (xmlStrcmp(auxnode->name, BAD_CAST "ref") == 0) {
								...
								if (text) {
									if (xmlStrcmp(text, BAD_CAST "dtd") == 0) {
										...
										if (text) {
											gchar *pstr =
												g_strdup_printf("%s/bfish_%ld", g_get_tmp_dir(),
																(long int) time(NULL));
											gchar *info;
											if (xmlNanoHTTPFetch((const char *) text, pstr, &info) == -1) {
												...
												infb_insert_error(GTK_TEXT_VIEW(widget),
																  BAD_CAST
																  _("Cannot load file from network"));
												/*message_dialog_new(bfwin->main_window,
											...
											doc = xmlReadFile(pstr, NULL,
															  ...
															  XML_PARSE_NOBLANKS | XML_PARSE_XINCLUDE |
															  XML_PARSE_NOWARNING | XML_PARSE_NOERROR);
											if (doc)
												doc->URL = xmlStrdup(BAD_CAST text);
											g_free(pstr);
										}
									} else
										xmlFree(text);
...
static void
infb_up_clicked(GtkButton * button, gpointer data)
{
	if (!infb_v.currentDoc || !data)
		return;
...
static void
infb_idx_clicked(GtkButton * button, gpointer data)
{
	if (!data)
		return;
...
static void
infb_midx_clicked(GtkButton * button, gpointer data)
{
	if (!data)
		return;
	if (infb_v.currentDoc != NULL && infb_v.currentDoc != infb_v.homeDoc)
...
static void
infb_save_clicked(GtkButton * button, gpointer data)
{
	gchar *pstr, *tofree;
	...
	if (infb_v.currentNode != NULL) {
		if (infb_v.currentType == INFB_DOCTYPE_DOCBOOK) {
			...
			tofree = pstr;
			pstr =
				g_strconcat(pstr, ":", infb_db_get_title(infb_v.currentDoc, FALSE, infb_v.currentNode), NULL);
			g_free(tofree);
			text = BAD_CAST pstr;
		...
		pstr = g_strdup_printf("%s/bfrag_%s_%ld", userdir, (gchar *) text, (long int) time(NULL));
		f = fopen(pstr, "w");
		if (f) {
			...
			if (infb_v.currentNode != fnode)
				xmlFreeNode(fnode);
			message_dialog_new(((Tinfbwin *) data)->bfwin->main_window, GTK_MESSAGE_INFO, GTK_BUTTONS_CLOSE,
							   _("Fragment saved"), (gchar *) text);
			g_free(pstr);
			xmlFree(text);
		} else {
			message_dialog_new(((Tinfbwin *) data)->bfwin->main_window, GTK_MESSAGE_INFO, GTK_BUTTONS_CLOSE,
							   _("Cannot open file"), (gchar *) pstr);
			g_free(pstr);
			xmlFree(text);
		}
	}
...
gboolean
infb_search_keypress(GtkWidget * widget, GdkEventKey * event, Tbfwin * bfwin)
{
	gchar *txt, *str;
	...
	if (txt && strcmp(txt, "") != 0) {
		...
		case INFB_DOCTYPE_HTML:
			if (win) {
				...
				if (win->search_tag) {
					gtk_text_buffer_get_bounds(gtk_text_view_get_buffer(GTK_TEXT_VIEW(win->view)), &mstart,
											   &mend);
					gtk_text_buffer_remove_tag_by_name(gtk_text_view_get_buffer(GTK_TEXT_VIEW(win->view)),
													   "search_tag", &mstart, &mend);
				}
				...
				if (gtk_text_iter_forward_search(&iter, txt, GTK_TEXT_SEARCH_TEXT_ONLY, &mstart, &mend, NULL)) {
					...
					if (!win->search_tag)
						win->search_tag =
							gtk_text_buffer_create_tag(gtk_text_view_get_buffer(GTK_TEXT_VIEW(win->view)),
													   "search_tag", INFB_STYLE_SEARCH, NULL);
					gtk_text_buffer_apply_tag(gtk_text_view_get_buffer(GTK_TEXT_VIEW(win->view)),
											  win->search_tag, &mstart, &mend);
					return FALSE;
				} else {
					message_dialog_new(bfwin->main_window, GTK_MESSAGE_INFO, GTK_BUTTONS_CLOSE,
									   _("Nothing found"), txt);
					return FALSE;
				}
			}
		...
		default:				/* fref2 */
			...
			str =
				g_strconcat("/descendant::note[contains(@title,\"", txt,
							"\") and local-name(..)!=\"element\"]", NULL);
			result = getnodeset(infb_v.currentDoc, BAD_CAST str, NULL);
			g_free(str);
			if (result) {
				found = TRUE;
...
void
infb_sidepanel_initgui(Tbfwin * bfwin)
{
	Tinfbwin *win;
	...
	gtk_menu_tool_button_set_arrow_tooltip_text(GTK_MENU_TOOL_BUTTON(win->saved),
												_("Go to selected fragment"));
...
void
infb_sidepanel_destroygui(Tbfwin * bfwin)
{
