...
static void bf_textview_delete_range_after_cb(GtkTextBuffer * textbuffer, GtkTextIter * arg1,
										GtkTextIter * arg2, gpointer user_data);										
static void  bf_textview_mark_set_cb(GtkTextBuffer *buf,GtkTextIter *arg1,GtkTextMark *arg2,gpointer widget);
...
GtkWidget *bf_textview_new(void)
{
   ...
   o->markset_signal_id = g_signal_connect_after(G_OBJECT(gtk_text_view_get_buffer(GTK_TEXT_VIEW(o))), 
   													"mark-set", G_CALLBACK(bf_textview_mark_set_cb), o);
	o->insert_signal_id = g_signal_connect_after(G_OBJECT(gtk_text_view_get_buffer(GTK_TEXT_VIEW(o))), 
	...
	o->internal_tags[IT_FOLDED] =	gtk_text_tag_table_lookup(gtk_text_buffer_get_tag_table
	...
	o->internal_tags[IT_FOLD_HEADER] = gtk_text_tag_table_lookup(gtk_text_buffer_get_tag_table
	...
	o->internal_tags[IT_BLOCK_MATCH] =	gtk_text_tag_table_lookup(gtk_text_buffer_get_tag_table
...
{ 
 /* insert set which means cursor position changed */
	...
	{		
		...
		gtk_text_buffer_remove_tag(buf, BF_TEXTVIEW(widget)->internal_tags[IT_BLOCK_MATCH], &it, &it2);
		if (bi->type == BI_START) 
		{
	...
	if ( block )
		...
		bi = NULL;	
	if ( block && bi->folded ) block = NULL;
	if (block) 
	{		
		gtk_text_buffer_get_iter_at_mark(buf, &it, block);
		gtk_text_buffer_get_iter_at_mark(buf, &it2,bi->ref);
		gtk_text_buffer_apply_tag(buf, BF_TEXTVIEW(widget)->internal_tags[IT_BLOCK_MATCH], &it, &it2);
		if (bi->type == BI_START) {
			gtk_text_buffer_get_iter_at_mark(buf, &it,bi->refe1);
			gtk_text_buffer_get_iter_at_mark(buf, &it2,bi->refe2);
	} 
...
while (gtk_text_iter_compare(&it, arg2) <= 0) 
{
	...
	if (mark) 
	{
		...
		if ( bi && !bi->folded ) 
		{
			if (bi->type == BI_START) 
			{
				mark2 = bi->ref; mark3=bi->refe1; mark4=bi->refe2;
					...
					&& gtk_text_iter_in_range(&it3, arg1, arg2)) 
				{
					if (!gtk_text_iter_has_tag(&it2, view->internal_tags[IT_BLOCK])) 
					{
						...
						gtk_text_buffer_remove_tag(textbuffer, view->internal_tags[IT_BLOCK], &it2, &it3);
					}
					if (bi->tagname) g_free(bi->tagname);
					if (bi2 && bi2->tagname) g_free(bi2->tagname);
					if (bi3 && bi3->tagname) g_free(bi3->tagname);
					if (bi4 && bi4->tagname) g_free(bi4->tagname);										
...
if (!bi || bi->single_line || bi->type != BI_START) return;
if (bi->folded) 
{
	bi->folded = FALSE;
...
static void bftv_scantable_insert(BfState *scantable, guint8 type, gpointer data,BfLangConfig *cfg) {
...
if (!ptr || !xmlCheckUTF8(ptr) ) return;
...
while ( i < size )    /* main loop */
{
	...
	else  /* REGULAR */	 
	{
		...
		reverse_set = FALSE;
		for (m = 0; m < BFTV_SCAN_RANGE; m++) 	charset[m] = FALSE;
		if (*input == '[') 
		{
			k = i + 1;
			inp3 = input + 1;
				...
				if ( charset[m] )
				{
					...
					{ 
						...
						else
						{
							...
							if ( !found ) g_free((BfState*)st->tv[m]);
							st->tv[m] = z;
						}		
					}	 
				} /* charset */
	...
	case ST_BLOCK_BEGIN:
		...
		if (tmps) 
		{
			...
			if (tmps2) 
			{
				...
				b->context = NULL;
			tmps2 = xmlGetProp(node, (const xmlChar *) "defaultstyle");
			b->tag = get_tag_for_scanner_style((gchar *) cfg->name, "b", (gchar *) b->name, (gchar *) tmps2);
			if (!b->tag) {
			...
			if ( b->context )
			{
				bftv_scantable_insert(&((BfLangBlock*)b->context)->scan_table,ST_BLOCK_BEGIN,b,cfg);
			}	
			else
			{
...
if (xmlStrcmp(cur->name, (const xmlChar *) "bflang") == 0) 
{
			...
			if (xmlStrcmp(cur2->name, (const xmlChar *) "option") == 0) 
			{
				...
				if (tmps) 
				{
					...
					else if (xmlStrcmp(tmps, (const xmlChar *) "autoclose-exclude") == 0)
					{
						...
						while (arr[i]) 
						{
							g_hash_table_insert(cfg->dont_autoclose,g_strdup(arr[i]),&const_true);
							i++;
						}
						g_strfreev(arr);
					}	
	...
	if (xmlStrcmp(cur->name, (const xmlChar *) "token") == 0) 
	{
		bftv_make_entity(doc, cur, cfg, ST_TOKEN, NULL, NULL, NULL);
	} /* token without a group */
	else 
	if (xmlStrcmp(cur->name, (const xmlChar *) "token-list") == 0) 
...
while ( lst )
{
	...
	if ( !bbb->markup )
	{
		if ( bbb->scan_table.tv[(gint)'<'] ==
				 cfg->scan_table.tv[(gint)'<']  )
...
g_return_if_fail(self != NULL);
g_return_if_fail(BF_IS_TEXTVIEW(self));
if (!self->lang || self->paste_operation )	 return;
its = *start;
ita = *start;
...
{	/* main loop */
	...
	while (gtk_text_iter_compare(&ita, end) <= 0 && self->lang->escapes[(gint) c]) 
	{	/* remove escapes */
							...
							if (!g_queue_is_empty(&(self->scanner.tag_stack)))	
							{
								...
								if (bf) 
								{
									...
									if (do_mark) 
									{
										...
										bi->type = BI_START; bi->folded = FALSE;
										bi->tagname = g_strdup(arr2[0]);
										bi->ref = mark2; bi->refe1=mark3; bi->refe2=mark4;
										g_object_set_data(G_OBJECT(mark), "bi", bi);
										BlockInfo *bi2 = g_new0(BlockInfo,1);
										bi2->type = BI_END; bi2->folded = FALSE;
										bi2->tagname = g_strdup(arr2[0]);
										bi2->ref = mark4; bi2->refb1=mark; bi2->refb2=mark2;
