...
static void
bluefish_encodings_dialog_get_property (GObject *object,
																				guint prop_id,
																				GValue *value,
																				GParamSpec *pspec)
{
...
static void
enc_avail_view_selection_changed (GtkTreeSelection *selection,
...
static void
enc_in_menu_view_selection_changed (GtkTreeSelection *selection,
																		BluefishEncodingsDialog *dialog)
{	
	if (gtk_tree_selection_count_selected_rows (selection) > 0) {
...
static void
enc_add_button_clicked (GtkButton *button,
												BluefishEncodingsDialog *dialog)
{
...
static void
enc_remove_button_clicked (GtkButton *button,
													 BluefishEncodingsDialog *dialog)
{
	GtkTreeIter childIter, filterIter;
	GtkTreeModel *model;
...
static void 
encodings_dialog_response_lcb (GtkDialog *dialog,
															 gint response,
															 BluefishEncodingsDialog *encDialog)
{
	GtkTreeIter iter;
	gboolean valid;
	GList *tmplist = NULL;
	...
	if (encDialog->priv->bfwin->current_document)
		menu_current_document_set_toggle_wo_activate (encDialog->priv->bfwin, NULL, encDialog->priv->bfwin->current_document->encoding);
...
{
  BluefishEncodingsDialogClass *klass = BLUEFISH_ENCODINGS_DIALOG_CLASS (g_type_class_peek (BLUEFISH_TYPE_ENCODINGS_DIALOG));
  ...
  GObject *obj = parent_class->constructor (type,
	...
	for (node = main_v->globses.encodings; node != NULL; node = node->next) {
		...
		if (g_strv_length (arr) == 3)
		{
			...
			if (g_ascii_strcasecmp (arr[1], "UTF-8") == 0 || g_list_find_custom (encodings, arr[1], (GCompareFunc) g_ascii_strcasecmp) != NULL)
				canRemove = FALSE;
	...
	gtk_widget_set_sensitive (dialog->priv->addButton, FALSE);
	g_signal_connect (dialog->priv->addButton, "clicked",
										G_CALLBACK (enc_add_button_clicked), dialog);
	...
	gtk_widget_set_sensitive (dialog->priv->removeButton, FALSE);
	g_signal_connect (dialog->priv->removeButton, "clicked",
										G_CALLBACK (enc_remove_button_clicked), dialog);
	...
	column = gtk_tree_view_column_new_with_attributes (_("Description"),
																										 ...
																										 COLUMN_DESCRIPTION,
																										 NULL);
	gtk_tree_view_append_column (GTK_TREE_VIEW (dialog->priv->encInMenuView),
															 column);
