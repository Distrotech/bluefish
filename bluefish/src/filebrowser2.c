...
static void DEBUG_DIRITER(Tfilebrowser2 *fb2, GtkTreeIter *diriter) {
	gchar *name;
	gtk_tree_model_get(GTK_TREE_MODEL(fb2->filesystem_tstore), diriter, FILENAME_COLUMN, &name, -1);
...
static GtkTreeIter *fb2_add_dir_entry(Tfilebrowser2 *fb2, GtkTreeIter *parent, GnomeVFSURI *child_uri, gint type) {
	GtkTreeIter *newiter;
	guint *hashkey;
	...
	*hashkey = gnome_vfs_uri_hash(child_uri);
	...
	if (newiter != NULL) {
		...
		gtk_tree_store_set(GTK_TREE_STORE(fb2->filesystem_tstore),newiter,REFRESH_COLUMN, 0,-1);
		g_free(hashkey);
	} else {
		gpointer pixmap = FILEBROWSERCONFIG(main_v->filebrowserconfig)->dir_icon;
		if (type != TYPE_DIR) {
			pixmap = FILEBROWSERCONFIG(main_v->filebrowserconfig)->unknown_icon;
		}
		newiter = g_new(GtkTreeIter,1);
		gtk_tree_store_append(GTK_TREE_STORE(fb2->filesystem_tstore),newiter,parent);
		DEBUG_MSG("fb2_add_dir_entry, will add %s\n",gnome_vfs_uri_extract_short_name(child_uri));
		gtk_tree_store_set(GTK_TREE_STORE(fb2->filesystem_tstore),newiter,
...
/*			if (finfo->type == GNOME_VFS_FILE_TYPE_DIRECTORY) {*/
				fb2_add_dir_entry(cdata->fb2, cdata->parent, child_uri, type);
/*			} else {
...
/*		fb2_liststore_delete_refresh1(cdata->fb2,cdata->fb2->file_lstore);*/
		fb2_treestore_delete_children_refresh1(cdata->fb2, cdata->fb2->filesystem_tstore, cdata->parent);
		g_free(cdata);
	}
}
...
static void fb2_refresh_dir(Tfilebrowser2 *fb2, GnomeVFSURI *uri, GtkTreeIter *dir) {
	...
	if (dir) {
		...
		tpath = gtk_tree_model_get_path(GTK_TREE_MODEL(fb2->filesystem_tstore),dir);
		refilter_filelist(fb2, tpath);
		gtk_tree_view_expand_to_path(GTK_TREE_VIEW(fb2->dir_v), tpath);
		gtk_tree_path_free(tpath);
		fb2_treestore_mark_children_refresh1(fb2->filesystem_tstore, dir);
...
static void fb2_build_dir(Tfilebrowser2 *fb2, GnomeVFSURI *uri) {
	...
	if (!parent) {
		...
		parent = fb2_add_dir_entry(fb2, NULL, tmp, TYPE_DIR);
		parent_uri = tmp;
	} else {
		parent_uri = tmp;
	}
	DEBUG_MSG("we should have a parent here!! parent(%p), and the uri for that parent is %s\n",parent,gnome_vfs_uri_get_path(parent_uri));
	{
		...
		while (!done) {
			...
			parent = fb2_add_dir_entry(fb2, parent, tmp2, TYPE_DIR);
...
static void refilter_filelist(Tfilebrowser2 *fb2, GtkTreePath *newroot) {
	...
	fb2->file_lfilter = gtk_tree_model_filter_new(GTK_TREE_MODEL(fb2->filesystem_tstore),newroot);
...
/*	fb2->file_itable = g_hash_table_new(g_int_hash,g_int_equal);*/
	fb2->filesystem_tstore = gtk_tree_store_new(N_COLUMNS,GDK_TYPE_PIXBUF,G_TYPE_STRING,G_TYPE_POINTER,G_TYPE_BOOLEAN,G_TYPE_INT);
/*	fb2->file_lstore = gtk_list_store_new(N_COLUMNS,GDK_TYPE_PIXBUF,G_TYPE_STRING,G_TYPE_POINTER,G_TYPE_BOOLEAN);*/
	...
	gtk_box_pack_start(GTK_BOX(vbox),fb2->dirmenu, FALSE, FALSE, 0);
	{
		...
		fb2->dir_tfilter = gtk_tree_model_filter_new(GTK_TREE_MODEL(fb2->filesystem_tstore),NULL);
		...
		fb2->file_lfilter = gtk_tree_model_filter_new(GTK_TREE_MODEL(fb2->filesystem_tstore),NULL);
		gtk_tree_model_filter_set_visible_func(GTK_TREE_MODEL_FILTER(fb2->file_lfilter),file_list_filter_func,fb2,NULL);
		fb2->file_lsort = gtk_tree_model_sort_new_with_model(GTK_TREE_MODEL(fb2->file_lfilter));
		gtk_tree_sortable_set_sort_column_id(GTK_TREE_SORTABLE(fb2->file_lsort),FILENAME_COLUMN,GTK_SORT_ASCENDING);
