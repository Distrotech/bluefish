...
void match_autocomplete_reference(Tscantable *st,guint16 matchnum, guint16 context) {
	...
	if (g_array_index(st->matches, Tpattern, matchnum).tagclose_from_blockstack && !g_array_index(st->contexts, Tcontext, context).has_tagclose_from_blockstack) {
...
/*	if (g_array_index(st->matches, Tpattern, matchnum).reference && !g_array_index(st->matches, Tpattern, matchnum).is_regex) {
	...
	if (g_array_index(st->matches, Tpattern, matchnum).autocomp_items) {
		...
		while(tmpslist) {
			...
			g_hash_table_insert(g_array_index(st->contexts, Tcontext, context).patternhash,pac->autocomplete_string,GINT_TO_POINTER(pattern_id));
