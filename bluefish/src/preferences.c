...
sessionprefs(const gchar * label, Tsessionprefs * sprefs, Tsessionvars * sessionvars)
{
	...
	sprefs->prefs[template] =
		dialog_combo_box_text_from_list_in_table(poplist, curtemplate ? curtemplate : _("None"), table, 1, 6,
												 1, 2);
...
preferences_apply(Tprefdialog * pd)
{
	...
	main_v->props.modified_check_type =
		gtk_option_menu_get_history(GTK_OPTION_MENU(pd->prefs[modified_check_type]));
...
preferences_dialog()
{
	...
	pd->prefs[editor_indent_wspaces] = dialog_check_button_in_table(_("Insert _spaces instead of tabs"),
																	main_v->props.editor_indent_wspaces,
																	table, 0, 1, 0, 1);
	...
	pd->prefs[modified_check_type] =
		boxed_optionmenu_with_value(_("File properties to check on dis_k for modifications"),
									main_v->props.modified_check_type, vbox2, modified_check_types);
